...
- [x] Get the computational microscope strategies (~2 hours, depends on number of participants):
   ...
   do condor_submit_bid 1 submission_scripts/MPI-IS/M_00_Get_CM_Strategies.sub experiment=methods_main save_path=/fast/vfelso
   ```
...
- [x] Once the inference is done for the participants, get the best parameters by running (~30 minutes):
   ...
   ```
- [ ] Pre-calculate trial-by-trial likelihoods (~30 minutes):
   ```
...
- [x]  At the same time, you can simulate new trajectories on the cluster (~30 minutes):
   ```
   cd <path to project>/cluster
   condor_submit_bid 1 submission_scripts/MPI-IS/06_Simulate_Trajectories.sub param_file=reduced
   condor_submit_bid 1 submission_scripts/MPI-IS/06_Simulate_Trajectories.sub param_file=no_added_cost policy=OptimalQ 
   ```
   Once methods main best parameters are found:
   ``` 
   condor_submit_bid 1 submission_scripts/MPI-IS/06_Simulate_Trajectories.sub param_file=participants_gamma,kappa 
   condor_submit_bid 1 submission_scripts/MPI-IS/06_Simulate_Trajectories.sub param_file=participants_back_added_cost,gamma,kappa
   condor_submit_bid 1 submission_scripts/MPI-IS/06_Simulate_Trajectories.sub param_file=participants_gamma,given_cost,kappa
   ```
...
- [ ] Once the simulation jobs are done, on the cluster, start the jobs to infer parameters for the simulations (can take a while, like 24 hours):
   ```
   cd <path to project>/cluster
   for file_idx in {1..9};
   ...
   ```
- [ ] Once the inference for the simulations are finished, combine the output files in the cluster (can take ~12+ hours):
   ```
   cd <path to project>/cluster
   for file_idx in {1..9};
...
- [ ] Next, get the best parameters:
  ...
  ```
- [ ] Calculate Optimal BIC data for later analyses:
   ```
   condor_submit_bid 1 submission_scripts/MPI-IS/M_02_Get_Optimal_BIC.sub`
   ```
...
|:------------------:|---------:|:------------------------------:|:------------------------------------------------------------------------------------------------------------------------------------------------:|------------------------------------------------------------------------------------------------------------------------------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
| :heavy_check_mark: |    Model |             Exp 1              |                                      Participant behavior was relatively stable for the last half of trials                                      | `python find_stable_point_with_cm.py -e MainExperiment`                                                                                                    |                                        `condor_submit_bid 1 submission_scripts/MPI-IS/M_03_Report_CM_Strategies.sub > log/CM_MainExperiment.stdout`                                         |
|                    |    Model |        Exp 1 (Table 2)         |          The model containing both a cost of prospection parameter and a mental effort parameter explains participants’ data best (BIC)          | `python plot_bic.py -e MainExperiment`                                                                                                                     |                                                `condor_submit_bid 1 submission_scripts/MPI-IS/M_04_BIC.sub > log/BIC_MainExperiment.stdout`                                                 |
|                    |    Model |       Exp 1 (Figure 3a)        |          The model containing both a cost of prospection parameter and a mental effort parameter explains participants’ data best (BMS)          | `python plot_bms.py -e MainExperiment`                                                                                                                     |                                                `condor_submit_bid 1 submission_scripts/MPI-IS/M_05_BMS.sub > log/BMS_MainExperiment.stdout`                                                 |
|                    |    Model |   Exp 1 (Figures 3b and 3c)    | The model containing both a cost of prospection parameter and a mental effort parameter explains participants’ data best (Trial Log Likelihoods) | `python plot_participant_average_likelihoods.py -e MainExperiment`                                                                                         |                                                         `condor_submit_bid 1 submission_scripts/MPI-IS/M_06_Click_Likelihoods.sub`                                                          |
|                    |    Model |             Exp 1              |                                            Behavior of simulated agents matches participant behavior                                             | `python posterior_predictive_check.py -e MainExperiment`                                                                                                   |                                                     `condor_submit_bid 1 submission_scripts/MPI-IS/M_07_Posterior_Predictive_Check.sub`                                                     |
|                    |    Model |             Exp 1              |                                       Understanding the environment’s structure mitigates experienced cost                                       | `python investigate_structure_knowledge.py -e MainExperiment`                                                                                              |                                                  `condor_submit_bid 1 submission_scripts/MPI-IS/M_08_Investigate_Structure_Knowledge.sub`                                                   |
|                    |   Method |          Simulations           |                                            Both cost parameters can be recovered from simulated data                                             | `python simulated_parameter_recovery.py -e SoftmaxRecovery`                                                                                                |                                                    `condor_submit_bid 1 submission_scripts/MPI-IS/M_09_Simulated_Parameter_Recovery.sub`                                                    |
|                    |   Method |          Simulations           |                                 Our method outputs individual parameters with high confidence for simulated data                                 | `python plot_simulated_hdi.py -e SoftmaxRecovery`                                                                                                          |                                                           `condor_submit_bid 1 submission_scripts/MPI-IS/M_10_Simulated_HDI.sub`                                                            |
|                    |   Method |             Exp 2              |          The manipulated cost of prospection parameter but not the mental effort parameter has an effect on planning operations chosen           | `python validation_experiment_validation.py -e ValidationCostModel`                                                                                        |                                                       `condor_submit_bid 1 submission_scripts/MPI-IS/M_11_Experiment_Validation.sub`                                                        |
|                    |   Method |             Exp 2              |           The cost of prospection parameter but not the mental effort parameter manipulation is reflected in block-wise MAP estimates            | `python regression_cross_validation.py -e ValidationExperiment`                                                                                            |                                                    `condor_submit_bid 1 submission_scripts/MPI-IS/M_12_Regression_Cross_Validation.sub`                                                     |
|                    |   Method |             Exp 2              |                         Our method can be used to classify individuals as exhibiting a high versus low cost of planning                          | `python plot_validation_hdi.py -e ValidationExperiment`                                                                                                    |                                                           `condor_submit_bid 1 submission_scripts/MPI-IS/M_13_Validation_HDI.sub`                                                           |
...
|                    | Appendix |         Method, Exp 2          |                                Participants with more spread out posteriors are still well explained by the model                                | `python plot_main_hdi.py -e MainExperiment`                                                                                                                |                                                         `condor_submit_bid 1 submission_scripts/MPI-IS/M_16_MainExperiment_HDI.sub`                                                         |
...
|                    | Appendix |             Exp 2              |                               Both blocks in Experiment #2 are fit just as well as the test block in Experiment #1                               | `python regression_cross_validation.py -e ValidationExperiment`                                                                                            |                                                    `condor_submit_bid 1 submission_scripts/MPI-IS/M_12_Regression_Cross_Validation.sub`                                                     |                         
|                    | Appendix |             Exp 2              |                                                       Highest posterior density intervals                                                        | `python plot_validation_hdi.py -e ValidationExperiment`                                                                                                    |                                                           `condor_submit_bid 1 submission_scripts/MPI-IS/M_13_Validation_HDI.sub`                                                           |
