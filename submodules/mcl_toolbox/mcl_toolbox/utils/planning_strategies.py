...
from ..env.modified_mouselab import approx_max
...
def observe_randomly_till_root(trial, node, rng=None):
...
def observe_leaves_of_root(trial, root, satisficing_value=None, rng=None):
    rng = np.random.default_rng(rng)
...
def get_max_leaves(trial, rng=None):
    rng = np.random.default_rng(rng)
...
def observe_nodes_satisficing(trial, nodes, satisficing_value, rng=None):
    rng = np.random.default_rng(rng)
    rng.shuffle(nodes)
...
def get_max_nodes(trial, nodes, rng=None):
    rng = np.random.default_rng(rng)
    ...
    rng.shuffle(max_nodes)
    return max_nodes


def observe_node_path_subtree(trial, node, random=True, rng=None):
    rng = np.random.default_rng(rng)
    ...
    if random:
        ...
        rng.shuffle(unobserved_successors)
...
def explore_subtree_random(subtree_root, threshold_value=None, rng=None):
    rng = np.random.default_rng(rng)
...
@strategy(1)
def goal_setting_random_path(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    for node in leaf_nodes:
        node.observe()
        if node.value > 0:
            observe_randomly_till_root(trial, node, rng=rng)
    for node in leaf_nodes:
        observe_randomly_till_root(trial, node, rng=rng)
...
@strategy(2)
def inverse_randomized_breadth_first_search(trial, rng=None):
    rng = np.random.default_rng(rng)
    max_depth = trial.max_depth
...
@strategy(3)
def randomized_breadth_first_search(trial, rng=None):
    rng = np.random.default_rng(rng)
    max_depth = trial.max_depth
...
@strategy(4)
def progressive_deepening(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    for node in leaf_nodes:
...
def best_first_search_expected_value(trial, rng=None):
    rng = np.random.default_rng(rng)
    pq = PriorityQueue()
    pq.put((-0, 0))
...
@strategy(6)
def ancestor_priority(trial, rng=None):
    ...
    while len(unobserved_nodes) != 0:
        ...
        node = rng.choice(max_total_nodes)
        node.observe()
...
@strategy(7)
def successor_priority(trial, rng=None):
    """Choose nodes according to the priority 0.8*number of
...
@strategy(8)
def backward_path_goal_setting(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    for node in leaf_nodes:
...
@strategy(9)
def backward_path_subtree_goal_setting(trial, rng=None):
    ...
    for node in leaf_nodes:
        if not node.observed:
            node.observe()
            if node.value > 0:
                observe_node_path_subtree(trial, node, rng=rng)
    for node in leaf_nodes:
        observe_till_root(trial, node)
    return [node.label for node in trial.observed_nodes] + [0]
...
@strategy(10)
def randomized_sibling_breadth_first_search(trial, rng=None):
    rng = np.random.default_rng(rng)
    max_depth = trial.max_depth
    for d in list(range(1, max_depth + 1)):
        nodes = trial.level_map[d]
...
@strategy(11)
def immediate_ancestor_priority(trial, rng=None):
    rng = np.random.default_rng(rng)
    unobserved_nodes = trial.unobserved_nodes.copy()
    unobserved_nodes.remove(trial.node_map[0])
    rng.shuffle(unobserved_nodes)
...
@strategy(12)
def immediate_successor_priority(trial, rng=None):
    rng = np.random.default_rng(rng)
    unobserved_nodes = trial.unobserved_nodes.copy()
    unobserved_nodes.remove(trial.node_map[0])
    rng.shuffle(unobserved_nodes)
...
@strategy(13)
def backward_path_immediate_goal_setting(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    for node in leaf_nodes:
        if not node.observed:
            node.observe()
            if node.value > 0:
                observe_node_path_subtree(trial, node, random=False, rng=rng)
    for node in leaf_nodes:
        observe_till_root(trial, node)
    return [node.label for node in trial.observed_nodes] + [0]
...
@strategy(14)
def best_leaf_node_path(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    pq = PriorityQueue()
...
@strategy(15)
def non_terminating_approximate_optimal(trial, rng=None):
...
@strategy(16)
def goal_setting_backward_planning(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    max_value = trial.get_max_dist_value()
...
@strategy(17)
def goal_setting_equivalent_goals_level_by_level(trial, rng=None):
    rng = np.random.default_rng(rng)
    best_nodes = get_max_leaves(trial, rng=rng)
    ...
    while 1:
        ...
        if len(parent_pointers) == 0:
            return [node.label for node in trial.observed_nodes] + [0]
        temp_pointers = parent_pointers
        rng.shuffle(temp_pointers)


@strategy(18)
def goal_setting_equivalent_goals_random(trial, rng=None):
    rng = np.random.default_rng(rng)
    best_nodes = get_max_leaves(trial, rng=rng)
...
@strategy(19)
def goal_setting_equivalent_goals_leaf_to_root(trial, rng=None):
    rng = np.random.default_rng(rng)
    best_nodes = get_max_leaves(trial, rng=rng)
    rng.shuffle(best_nodes)
    for node in best_nodes:
...
@strategy(20)
def goal_setting_equivalent_goals_root_to_leaf(trial, rng=None):
    rng = np.random.default_rng(rng)
    best_nodes = get_max_leaves(trial, rng=rng)
    rng.shuffle(best_nodes)
    for node in best_nodes:
...
@strategy(21)
def approximate_optimal(trial, rng=None):
    rng = np.random.default_rng(rng)
    max_value = trial.get_max_dist_value()
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
...
@strategy(22)
def positive_forward_satisficing(trial, rng=None):
...
@strategy(25)
def satisficing_best_first_search_expected_value(trial, rng=None):
    rng = np.random.default_rng(rng)
    pq = PriorityQueue()
    pq.put((-0, 0))
    rf = trial.reward_function
...
@strategy(26)
def backward_positive_satisficing(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    for node in leaf_nodes:
...
@strategy(27)
def one_final_outcome(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
...
@strategy(28)
def one_immediate_outcome(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
...
@strategy(29)
def goal_setting_forward_planning(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    max_value = trial.get_max_dist_value()
...
@strategy(30)
def no_planning(trial, rng=None):
    rng = np.random.default_rng(rng)
    return [0]


@strategy(31)
def satisficing_dfs(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    max_value = trial.get_max_dist_value()
...
@strategy(32)
def positive_root_leaves(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
    ...
    for root in root_nodes:
        if root.value > 0:
            observe_leaves_of_root(trial, root, rng=rng)
    return [node.label for node in trial.observed_nodes] + [0]


@strategy(33)
def satisficing_positive_root_leaves(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
    max_value = trial.get_max_dist_value()
    ...
    for root in root_nodes:
        if root.value > 0:
            res = observe_leaves_of_root(
                trial, root, satisficing_value=max_value, rng=rng
            )
            if res == 1:
                break
    return [node.label for node in trial.observed_nodes] + [0]
...
@strategy(34)
def positive_satisficing_positive_root_leaves(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
    for root in root_nodes:
        root.observe()
        if root.value >= 0:
            observe_leaves_of_root(trial, root, satisficing_value=0, rng=rng)
            break
    return [node.label for node in trial.observed_nodes] + [0]


@strategy(35)
def backward_planning_positive_outcomes(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
...
@strategy(36)
def best_first_search_observed_roots(trial, rng=None):
...
@strategy(37)
def satisficing_best_first_search_observed_roots(trial, rng=None):
    # Currently implemented as starting after looking at all roots
    rng = np.random.default_rng(rng)
    pq = PriorityQueue()
    root_nodes = trial.node_map[0].children.copy()
...
@strategy(38)
def loss_averse_backward_planning(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    max_value = trial.get_max_dist_value()
    rng.shuffle(leaf_nodes)
...
@strategy(39)
def random_planning(trial, rng=None):
    rng = np.random.default_rng(rng)
...
@strategy(40)
def extra_optimal_planning(trial, rng=None):
    rng = np.random.default_rng(rng)
    max_value = trial.get_max_dist_value()
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
...
@strategy(41)
def leave_out_one_leaf(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
...
@strategy(42)
def explore_roots_non_optimal(trial, rng=None):
    rng = np.random.default_rng(rng)
    max_value = trial.get_max_dist_value()
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
...
@strategy(43)
def consecutive_second_extra(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
...
@strategy(44)
def explore_one_subtree_random(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
    subtree_root = root_nodes[0]
    explore_subtree_random(subtree_root, rng=rng)
    return [node.label for node in trial.observed_nodes] + [0]


@strategy(45)
def leave_one_subtree_random(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
...
@strategy(46)
def explore_all_subtrees_random(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
...
@strategy(47)
def explore_all_subtrees_termination(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
    max_value = trial.get_max_dist_value()
    for subtree_root in root_nodes:
        res = explore_subtree_random(subtree_root, max_value, rng=rng)
        if res == 1:
            break
    return [node.label for node in trial.observed_nodes] + [0]
...
@strategy(48)
def explore_roots_and_leaves(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(root_nodes)
...
@strategy(49)
def explore_roots_leaves_termination(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    leaf_nodes = trial.get_leaf_nodes()
    max_value = trial.get_max_dist_value()
...
@strategy(50)
def one_above_max(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    max_value = trial.get_max_dist_value()
...
@strategy(51)
def root_of_max(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    max_value = trial.get_max_dist_value()
...
@strategy(52)
def extra_positive_planning(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    count = 0
...
@strategy(53)
def leave_one_root(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
...
@strategy(54)
def explore_roots_subtrees(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
    observe_nodes(root_nodes)
    rng.shuffle(root_nodes)
    for node in root_nodes:
        explore_subtree_random(node, rng=rng)
    return [node.label for node in trial.observed_nodes] + [0]


@strategy(55)
def explore_num_roots_leaves(trial, rng=None):
    rng = np.random.default_rng(rng)
...
@strategy(56)
def explore_different_subtree_leaves(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
    for node in root_nodes:
        successor_leaves = get_subtree_leaves(node)
        rng.shuffle(successor_leaves)
        successor_leaves[0].observe()
    return [node.label for node in trial.observed_nodes] + [0]


@strategy(57)
def consecutive_second(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    second_best = get_second_max_dist_value(trial)
    previous = False
...
@strategy(58)
def double_extra_positive_planning(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    count = 0
...
@strategy(59)
def one_subtree_leaves(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
    explore_subtree_random(root_nodes[0], rng=rng)
...
@strategy(60)
def double_subtree_leaves(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
    explore_subtree_random(root_nodes[0], rng=rng)
    explore_subtree_random(root_nodes[1], rng=rng)
...
@strategy(61)
def explore_one_path(trial, rng=None):
    rng = np.random.default_rng(rng)
    branches = list(trial.branch_map.values())
    selected_branch = rng.choice(branches)
...
@strategy(62)
def explore_leaves_and_roots(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(root_nodes)
...
@strategy(63)
def explore_leaves_and_roots_termination(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(root_nodes)
...
@strategy(64)
def root_leaves_termination(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    max_value = trial.get_max_dist_value()
    rng.shuffle(root_nodes)
...
@strategy(65)
def root_leaves_termination_positive(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    max_value = 0
    rng.shuffle(root_nodes)
...
@strategy(66)
def explore_single_mid_node(trial, rng=None):
    rng = np.random.default_rng(rng)
    mid_nodes = get_mid_nodes(trial)
    rng.shuffle(mid_nodes)
    mid_nodes[0].observe()
    return [node.label for node in trial.observed_nodes] + [0]


@strategy(67)
def best_leaf_single_node_path(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    observe_nodes(leaf_nodes)
...
@strategy(68)
def best_leaf_complete_path(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    observe_nodes(leaf_nodes)
...
@strategy(69)
def best_subtree(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    observe_nodes(root_nodes)
    max_value = max([node.value for node in root_nodes])
    max_nodes = [node for node in root_nodes if node.value == max_value]
    selected_node = rng.choice(max_nodes)
    explore_subtree_random(selected_node, rng=rng)
    return [node.label for node in trial.observed_nodes] + [0]


@strategy(70)
def best_root_child(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
    observe_nodes(root_nodes)
    ...
    selected_child = rng.choice(children)
    selected_child.observe()
    return [node.label for node in trial.observed_nodes] + [0]


@strategy(71)
def root_leaf_mid(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    leaf_nodes = trial.get_leaf_nodes()
    mid_nodes = get_mid_nodes(trial)
...
@strategy(72)
def mid_root_leaf(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    leaf_nodes = trial.get_leaf_nodes()
    mid_nodes = get_mid_nodes(trial)
...
@strategy(73)
def explore_two_leaves(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
...
@strategy(74)
def explore_all_mid_nodes(trial, rng=None):
    rng = np.random.default_rng(rng)
    mid_nodes = get_mid_nodes(trial)
    rng.shuffle(mid_nodes)
    observe_nodes(mid_nodes)
...
@strategy(75)
def explore_best_leaf_roots(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    observe_nodes(leaf_nodes)
...
@strategy(76)
def explore_random_max_path(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    max_value = trial.get_max_dist_value()
...
@strategy(77)
def positive_extra_leaf(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    count = 0
...
@strategy(78)
def positive_parent_leaf(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    rng.shuffle(leaf_nodes)
    for node in leaf_nodes:
...
@strategy(79)
def explore_positive_subtrees(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
    observe_nodes(root_nodes)
    positive_nodes = [node for node in root_nodes if node.value > 0]
    rng.shuffle(positive_nodes)
...
@strategy(80)
def general_root_leaves(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    max_value = trial.get_max_dist_value()
    rng.shuffle(root_nodes)
...
@strategy(81)
def positive_root_path(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
    for node in root_nodes:
        ...
        if node.value > 0:
            successor_leaves = get_subtree_leaves(node)
            chosen_leaf = rng.choice(successor_leaves)
...
@strategy(82)
def breadth_first_search_termination(trial, rng=None):
    rng = np.random.default_rng(rng)
    max_depth = trial.max_depth
    max_value = trial.get_max_dist_value()
...
@strategy(83)
def explore_siblings(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    max_value = trial.get_max_dist_value()
    rng.shuffle(root_nodes)
...
@strategy(84)
def explore_subtree_mid(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    max_value = trial.get_max_dist_value()
    rng.shuffle(root_nodes)
    ...
    for root in root_nodes:
        if max_found:
            selected_child = rng.choice(root.children)
...
@strategy(85)
def explore_center_leaves(trial, rng=None):
    rng = np.random.default_rng(rng)
    leaf_nodes = trial.get_leaf_nodes()
    mid_nodes = get_mid_nodes(trial)
    rng.shuffle(leaf_nodes)
...
@strategy(86)
def mid_subtree_leaf(trial, rng=None):
    rng = np.random.default_rng(rng)
    mid_nodes = get_mid_nodes(trial)
    rng.shuffle(mid_nodes)
    for node in mid_nodes:
        node.observe()
        selected_child = rng.choice(node.children)
        selected_child.observe()
    return [node.label for node in trial.observed_nodes] + [0]


@strategy(87)
def leaves_middle_subtrees(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
    for node in root_nodes:
        ...
        rng.shuffle(leaves)
        observe_nodes(leaves)
        selected_leaf = rng.choice(leaves)
        selected_leaf.parent.observe()
    return [node.label for node in trial.observed_nodes] + [0]


@strategy(88)
def leaves_middle_subtrees_satisficing(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
    max_value = trial.get_max_dist_value()
...
@strategy(89)
def leaves_middle_subtree(trial, rng=None):
    rng = np.random.default_rng(rng)
    root_nodes = trial.node_map[0].children.copy()
    rng.shuffle(root_nodes)
    node = root_nodes[0]
